...
/**
 *\brief Macro that creates the test_solve_square structure and starts testing
 */
#define TEST_SOLVE_SQUARE(a, b, c, x1, x2, nRoots)                                             \
    ({                                                                                         \
        struct test_solve_square t = { { a, b, c }, { x1, x2 }, nRoots };                      \
        run_test_solve_square(t, tmp, &correct_answers, &all_answers);                         \
    })
